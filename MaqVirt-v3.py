...
class start(MyFrame1):
    ...
    def b_loaodOnButtonClick(self, envent):
        ...
        self.mv.readCode(self.m_filePicker1.GetPath())        
        self.mv.bringToMemory()
        # Carga los nemonicos a la memoria RAM  
        row = 0
        for i in self.mv.memory.memory.values():
             self.m_grid1.SetCellValue(row,0,str(i))
             row = row + 1
        # Carga los nombres de variables            
        for i in self.mv.memory.tablaVar:
             self.m_grid1.SetCellValue(int(self.mv.memory.tablaVar[i]),1,i)
    ...
    def b_ejecutarOnButtonClick(self, envent):
        ...
        self.mv.execute(delay,MyFrame1)
...
"""
# Frame de cifrado   
class frame_p(MyFrame1):
    def m_button2OnButtonClick(self, envent):
        texto = self.m_textCtrl1.GetValue()
        textoCifrado = cifradoPorSustitucion(texto,binario)
        self.cifrado.SetValue(textoCifrado)

# Frame de descifrado
class descifrator(MyFrame2):
    def m_button4OnButtonClick(self, envent):
        #wx.MessageBox("Muy bien")
        texto = self.txtC.GetValue()
        textoDescifrado = desCifraSustitucion(texto,binario)
        self.descifrado.SetValue(textoDescifrado)
